...
void Vgg(Graph& g) {
    ...
    int pipe_id = g.add_pipeline();

    int batch_size(64), channels(3), data_height(224), data_width(224);

    auto data = std::make_shared<DataOp>(batch_size, channels, data_height, data_width);
    g.add_op("input", data, pipe_id);
    ...
    g.add_op("fc7", fc7, pipe_id);

    auto relu7 = std::make_shared<ReLUOp>(slope, fc7);
    g.add_op("relu7", relu7, pipe_id);
    ...
    g.add_op("fc8", fc8, pipe_id);

    auto prob = std::make_shared<SoftMaxOp>(fc8);
    g.add_op("prob", prob, pipe_id);
}
